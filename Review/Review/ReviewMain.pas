...
{
To Do:
  - Использовать FileMapping для GDI+, раз уж он все равно создается...

  + Повторное чтение файла при быстром листании (для показа Info?)  

  * Поддержка JpegLIB
    + Поиск DLL
    + Ошибка преждевременного закрытия
    * Обработка ошибок
      + Если не декодировал JpegLIB - пусть попробует GDI+
      + JpegLIB - декодирование битых изображений
      + Циклическое декодирование Thumbnail
    + x64
    + Отдельный Internal декодер - наследник

  + Отображение без блокировки файла
    + Закрытие Mapped файла?
    + Сообщение об ошибке при переключении страниц
    + GFL: Не работает листание многостраничных изображений (только GIF?)

  + Сохранение повернутых изображений
    * AV при сохранении, одновременно с декодированием...
    x Файл заблокирован под x64 - проблема с Viewer'ом.
    + Не сохраняется поворот под x64
    + Ошибка при отсутствии файла
    + Не переписывать Read-Only файлы
    + Сохранение через GDI+ для PVD декодеров (по возможности)
...
Ready:
  ...
  * Видео:
  ...
  + Не работает декодирование на размер экрана для многостраничных изображений (?)
  + Кэширование замедляет листание? (Заменил SendMessage на PostMessage)

  + AV при работе декодера EMF.

  + Сброс ориентации при масштабировании
    + Ориентация в режиме Preview
